...
{
public:
  vector<POINT> pointsEdgeLeft;     // 赛道左边缘点集，从下向上存
  vector<POINT> pointsEdgeRight;    // 赛道右边缘点集，从下向上存
  ...
  uint16_t rowCutUp = 10;           // 图像顶部切行，默认为10最大为ROWSIMAGE / 4：240/4
  uint16_t rowCutBottom = 10;       // 图像底部切行，默认为10最大为ROWSIMAGE / 4：240/4
  ...
  void trackRecognition(bool isResearch, uint16_t rowStart)
  {
    ...
    int counterBlock = 0;                          // 色块计数器（行）记录色块的数量
    ...
    //  开始识别赛道左右边缘
    for (int row = rowStart; row > rowCutUp; row--) // 有效行：10~220（去除最上面rowCutUp行）
    {
      ...
      if (imageType == ImageType::Rgb) // 输入RGB图像
      {
        ...
        {
          if (imagePath.at<Vec3b>(row, col)[2] > 0 &&//[2]为红的通道
              imagePath.at<Vec3b>(row, col - 1)[2] == 0)
          {
            startBlock[counterBlock] = col;//找出由非红色变成红色的第一个点(红色的开始)
          }
          else
          {
            if (imagePath.at<Vec3b>(row, col)[2] == 0 &&
                imagePath.at<Vec3b>(row, col - 1)[2] > 0)
            {
              endBlock[counterBlock++] = col;//红色的结束（counterblock记录有多少个色块）
              if (counterBlock >= end(endBlock) - begin(endBlock))//超出最大范围
                break;
            }
          }
        }
  ...
  /**
   * 计算边缘斜率的标准差，像素的数量不足图像的1/4时返回1000，否则每隔10像素取一个点计算标准差，计算失败返回0
   */
