...
if (pointsEdgeRight.empty()) {
    RP = 0; // 或其他默认值
    if (textDebug) cout << "pointsEdgeRight为空，使用默认RP值" << endl;
} else {
    RP = pointsEdgeRight[0].x;
}
/*************************************************************************** */
    ...
    LP = 0; // 或其他默认值
    if (textDebug) cout << "pointsEdgeLeft为空，使用默认LP值" << endl;
} else {
    LP = pointsEdgeLeft[0].x;
}
 widthblock = vector<int>(max(R_Start.x, L_Start.x) + 1, -1);
        // if (textDebug)
        // {
        //     cout << "初始化widthblock:" << endl;
        //     for (int i = 0; i < widthblock.size(); i++)
        //         printf("widthblock[%3d] : %3d\n", i, widthblock[i]);
        // }
        for (int i = 0; i < pointsEdgeRight.size(); i++)
        {
            if (textDebug)
            {
                printf("pointsEdgeRight[%3d] : x = %3d, y = %3d\n", i, pointsEdgeRight[i].x, pointsEdgeRight[i].y);
            }
            widthblock[pointsEdgeRight[i].x] = pointsEdgeRight[i].y;
        }
        for (int i = 0; i < pointsEdgeLeft.size(); i++)
        {
            if (textDebug)
            {
                printf("pointsEdgeLeft[%3d] : x = %3d, y = %3d\n", i, pointsEdgeLeft[i].x, pointsEdgeLeft[i].y);
            }
            if (widthblock[pointsEdgeLeft[i].x] != -1)
                widthblock[pointsEdgeLeft[i].x] = (widthblock[pointsEdgeLeft[i].x] + pointsEdgeLeft[i].y) / 2;
        }
        // else
        //     widthblock[pointsEdgeLeft[i].x] = widthblock[pointsEdgeLeft[i].x];
        if (textDebug)
            for (int i = 0; i < widthblock.size(); i++)
                printf("widthblock[%3d] = %3d\n", i, widthblock[i]);
/******************************************************************************** */
    ...
    void trackRecognition(Mat &imageBinary, Mat &img)
    {
